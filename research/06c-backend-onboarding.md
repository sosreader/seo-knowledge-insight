...
| 順序 | 檔案 | 看什麼 |
|------|------|--------|
| 1 | `src/routes/chat.ts` | `isAgentEnabled()` 如何決定走 agent 還是 single-pass |
...
**目標**：理解 `route → isAgentEnabled → agent-loop（while-loop → tool_calls → executor → tool results → LLM）→ 回應` 這條進階主線。
...
└── utils/                # 純函式工具
    ...
    ├── mode-detect.ts          hasOpenAI() / hasSupabase()
