...
[問題] 「上次開會說 Discover 流量下降，怎麼解決？」
   ↓
[Retrieval] 把問題轉成 embedding → 在 1,317 筆 Q&A 中找最相關的 5 筆
   ↓
...
### 本專案 Retrieval 評估基準線（v2.12，2026-03-05）

| 指標                | 數值        | 說明                                 | 目標      |
| ------------------- | ----------- | ------------------------------------ | --------- |
| KW Hit Rate         | **73%**     | 查詢的關鍵字有多少被 top-5 覆蓋      | ≥ 85%（中間目標 78%+）|
...
| 2026-03-05 | 多來源擴充（**現行**）| **1,317** | v2.12：Notion 584 + Medium 505 + iThome 185 + Google Cases 43 |
...
| **Hit Rate**（Recall@K）                          | top-K 結果中有沒有包含正確答案           | Hit@5 = 前 5 筆有沒有                | **73%（KW）/ 80%（Category）** |
