...
## 26. RAG 迭代改進框架（v2.11，2026-03-05）
...
> 三層改進方向：Synonym 擴充 → Contextual Embeddings → Re-ranking
...
- 輸入：`output/qa_enriched.json`（655 items）
- 輸出：`output/qa_context.json`（`{ stable_id: context_string }`）
- 增量式：預設跳過已存在，`--force` 重跑
- 成本估算：655 × 200 tokens ≈ $0.05（有 prompt caching 時更低）
