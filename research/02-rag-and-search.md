...
### 本專案 Retrieval 評估基準線（v2.0+cjk，2026-03-02）

| 指標                | 數值       | 說明                                 | 目標      |
| ------------------- | ---------- | ------------------------------------ | --------- |
| KW Hit Rate         | **74%**    | 查詢的關鍵字有多少被 top-5 覆蓋      | ≥ 85%     |
| Category Hit Rate   | **80%**    | top-5 中有沒有正確分類的 Q&A         | ≥ 80% ✅  |
| MRR                 | **0.87**   | 第一筆正確結果的排名倒數（越高越好） | ≥ 0.85 ✅ |
| LLM Top-1 Precision | **100%** ✅ | top-1 結果是否真的和查詢相關        | —         |
| Precision@K         | **76%**    | top-K 中有幾個是 relevant（v2.11）  | ≥ 80%     |
| F1 Score            | **0.73**   | Precision/Recall 調和平均（v2.11）  | ≥ 0.78    |
...
| 2026-03-01 | 去重後（**現行**）| **655** | v2.0 最終版（+ 12 Google Cases）|
...
### Eval 整合（`POST /api/v1/eval/reranking`）

```bash
# 評估 reranker 效果
curl -X POST /api/v1/eval/reranking \
  -d '{"query": "如何改善 CTR", "candidates": [...], "top_k": 5}'
```

回傳重排後的候選清單（rank + reason）。
