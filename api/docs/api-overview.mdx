---
title: API Overview
description: All API endpoints at a glance
---

# API Overview

The API has 9 routers and 32 endpoints. All endpoints under `/api/v1/*` require `X-API-Key` authentication.
...
- `source_type` — `notion` / `medium` / `ithelp` / `google-cases`
...
| Method | Path | Description |
|--------|------|-------------|
| POST | `/api/v1/feedback` | Submit feedback on chat responses |

### Pipeline — 14 endpoints
...
| POST | `/api/v1/pipeline/fetch-articles` | Trigger article fetch (Medium + iThome + Google) |
| POST | `/api/v1/pipeline/extract-qa` | Trigger Q&A extraction |
| POST | `/api/v1/pipeline/dedupe-classify` | Trigger dedup + classification |
| POST | `/api/v1/pipeline/metrics` | Parse SEO metrics from CSV |
