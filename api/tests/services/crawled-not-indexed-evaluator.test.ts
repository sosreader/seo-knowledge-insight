...
import { evaluateIndexingAnalysis } from "../../src/services/indexing-evaluator.js";
import { parseIndexingTsv } from "../../src/services/indexing-parser.js";
import { analyzeIndexing } from "../../src/services/indexing-analyzer.js";
...
describe("evaluateIndexingAnalysis", () => {
  ...
  it("scores well for analyzer-generated markdown", () => {
    ...
    const result = evaluateIndexingAnalysis(insight.markdown, data);
  ...
  it("detects path coverage correctly", () => {
    ...
    const result = evaluateIndexingAnalysis(partialContent, data);
  ...
  it("detects severity assessment", () => {
    ...
    const result = evaluateIndexingAnalysis(withSeverity, data);
    expect(result.has_severity_assessment).toBe(1);

    const withoutSeverity = "一些路徑的資料如下。";
    const result2 = evaluateIndexingAnalysis(withoutSeverity, data);
  ...
  it("handles data with no paths", () => {
    const emptyData = parseIndexingTsv("全網域\t10%\t0\t100\t50\t\t80");
    const result = evaluateIndexingAnalysis("全網域分析", emptyData);
