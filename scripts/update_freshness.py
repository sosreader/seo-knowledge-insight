...
def _patch_batch(
    ...
    """PATCH a batch of freshness_score updates (individual PATCH per row)."""
    headers = {
        "apikey": service_key,
        "Authorization": f"Bearer {service_key}",
    ...
    for u in updates:
        resp = requests.patch(
            f"{supabase_url}/rest/v1/qa_items?id=eq.{u['id']}",
            headers=headers,
            json={"freshness_score": u["score"]},
            timeout=30,
        )
        if resp.status_code in (200, 204):
            ok += 1
        else:
            logger.error("Patch failed for %s: %s %s", u["id"], resp.status_code, resp.text[:200])
            fail += 1
    return ok, fail
