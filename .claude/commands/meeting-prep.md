...
### Off-Page Authority 指標表（外部第三方，月度更新）

| 指標 | 工具 | 重要性 | 衡量範圍 | 取得方式 |
|------|------|--------|---------|---------|
| DA (Domain Authority) | Moz | ⭐ 重要 | 1-100 全網域權威 | Moz Link Explorer 公開查詢 / WebSearch `"<domain> Moz DA"` |
| PA (Page Authority) | Moz | — | 1-100 單頁權威 | Moz Link Explorer |
| DR (Domain Rating) | Ahrefs | ⭐ 重要 | 1-100 反向連結強度 | Ahrefs Site Explorer 公開查詢 / WebSearch `"<domain> Ahrefs DR"` |
| AS (Authority Score) | Semrush | — | 1-100 綜合權威 | Semrush 公開查詢 |
| TF (Trust Flow) | Majestic | — | 1-100 信任流 | Majestic Site Explorer |
| CF (Citation Flow) | Majestic | — | 1-100 引用流 | Majestic Site Explorer |
| Monthly Traffic (UU) | Similarweb / Semrush | — | 月流量估值 | Similarweb / Semrush 公開查詢 |
| AI Visibility | Semrush | — | 1-100 在 AI 搜尋的能見度 | Semrush AI Toolkit |

**用途**：作為 S6 Authoritativeness、S7 連結生態評分的客觀錨點，避免完全依靠 GSC referral 等內部指標的主觀解讀。

**取得策略（Claude Code as LLM 模式）**：
- 第三方工具 API 不直接呼叫（需付費 / 需 token），改用 WebSearch 抓取公開值
- 重要指標：DA + DR 為必填；其他為選填
- 月度更新一次即可（變化緩慢），週報不重複追蹤
- 有效範圍：`vocus.cc` 主域名；個別作者頁的 DA/PA 細節屬深度分析延伸
...
**B7：Off-Page Authority 指標查詢（月度更新，跨報告 carry forward）**

讀取 `data/off-page-authority.jsonl`（最後一筆）；若 ≤30 天直接 carry forward，否則重新查詢：
```
WebSearch: "vocus.cc Moz Domain Authority"
WebSearch: "vocus.cc Ahrefs Domain Rating DR"
WebSearch: "vocus.cc Semrush Authority Score"
```
目的：為 S6 Authoritativeness、S7 連結生態評分提供客觀錨點。
- 必查：DA (Moz) + DR (Ahrefs)
- 選填：AS (Semrush) / TF + CF (Majestic) / Monthly Traffic / AI Visibility
- 取得後 append 一行至 `data/off-page-authority.jsonl`：
  ```bash
  echo '{"date":"YYYY-MM-DD","domain":"vocus.cc","da":<n>,"dr":<n>,"as":<n>,"tf":<n>,"cf":<n>,"monthly_traffic":<n>,"ai_visibility":<n>,"source":"WebSearch"}' >> data/off-page-authority.jsonl
  ```
- 若搜尋無結果或數值來源矛盾，標記「無法取得」，降級以 GSC referral 趨勢作 fallback；S6/S7 評分時改採主觀依據並註明

記錄到 `industryMap` 的 `offPageAuthority` 區塊。

將 B1-B7 結果匯集為 `industryMap`（15-22 筆動態）。
...
**Authoritativeness 評分客觀錨點（必填，引用 B7）**：

| DA / DR / AS 區間 | 對應 Authoritativeness 分數 | 說明 |
|------------------|--------------------------|------|
| 全部 ≥ 70 | 5（強） | 業界領先，引用率高 |
| 主要 50-70 | 4 | 行業中段以上，多數媒體認可 |
| 主要 30-50 | 3 | 行業中段，部分專業領域認可 |
| 主要 15-30 | 2 | 起步期，少量外部引用 |
| 全部 < 15 | 1（弱） | 無顯著外部認可 |

跨區間時採加權平均；若 GSC referral 趨勢與 DA/DR 趨勢矛盾（如 DA 上升但 referral 下降），標記「短期擾動 vs 長期權威分離」並在依據中說明。

若 B7 取得失敗，註明「無客觀錨點，純主觀評分」並引用 GSC referral / 業界提及作 fallback。

**Section 7：人本七要素分析** — Delta 指引同 S6。

**連結生態（Link Ecosystem）評分客觀錨點（引用 B7）**：

| DR (Ahrefs) 區間 | TF/CF 比 | 連結生態分數 |
|-----------------|---------|------------|
| ≥ 60 | > 0.8 | 5（健康） |
| 40-60 | 0.5-0.8 | 4 |
| 25-40 | 0.3-0.5 | 3 |
| 15-25 | 0.2-0.3 | 2 |
| < 15 或 TF/CF < 0.2 | — | 1（不健康） |

若 TF/CF < 0.5（trust 流低於 citation 流），代表外部連結品質低於數量、可能有 spammy 連結 — 標注於評分依據。

**Authoritativeness vs 連結生態的差異**：S6 評估「外部如何看本站」（媒體引用、行業認可），S7 評估「連結結構是否健康」（信任品質、來源多樣性）。同一份 DA/DR/TF 數據在兩個維度從不同角度解讀，避免重複扣分。
...
| B7 Authority 錨點 | DA + DR 必填或註明「無法取得」 | 補查或註明 fallback 依據 |
| S6/S7 客觀錨點 | Authoritativeness 與連結生態評分依據引用 B7 數值 | 補引用或註明降級主觀評分 |
...
#### F5：Off-Page Authority 結構化記錄（data/off-page-authority.jsonl）

若 B7 本次取得新數值（非 carry forward），append 一行：

```bash
echo '{"date":"YYYY-MM-DD","domain":"vocus.cc","da":<n>,"dr":<n>,"as":<n>,"tf":<n>,"cf":<n>,"monthly_traffic":<n>,"ai_visibility":<n>,"source":"WebSearch"}' >> data/off-page-authority.jsonl
```

若數值為 carry forward（≤30 天），跳過此步驟，並在報告中註明「沿用 YYYY-MM-DD 取得值」。

---

## Citation 標記規則

Perplexity 風格 `[N]` 標記。**目標 15-18 筆，安全區 13-22 筆。**

> ⚠️ `citation_count_in_range` 是 **bell curve 不是門檻**：`score = 1 − |count − 17.5| / 17.5`。
> **35 筆得 0.0000，與完全不引用同分**；26 筆只有 0.5143、23 筆 0.6857、12 筆 0.6857——**12 與 23 都會 FAIL**（門檻 0.7 對應 `12.25 ≤ c ≤ 22.75`，整數安全區為 13-22）。「證據越多越好」在過峰後反轉為懲罰。
> 2026-07-27 與 2026-09-04 各踩過一次（後者初版 35 筆），詳見 `research/03-evaluation.md` §25.4 與 §26.2。
>
> 超量時的裁剪程序（避免破壞可追溯性）：統計各 `[N]` 的 inline 使用次數 → 零使用者直接砍 →
> 有使用但要砍的映射到**同分類中語意最近的保留項** → 組成單一 `old → final` 映射用**一次**
> `re.sub` 套用（循序重編號會碰撞）→ 收斂 `[16][16]` 這類重複 → 斷言 inline 與清單雙向差集皆為空。
> 副作用：裁掉的通常正是從未 inline 引用的那些，`inline_citation_coverage` 會一併回升。
