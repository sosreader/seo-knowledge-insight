...
**Section 深度均衡規範**：Section 一限制 5-6 個因果區塊（不要超過 7 個），避免過長。Section 三（技術 SEO）、四（檢索未索引）、五（搜尋意圖）需增加推理深度：每個至少 5 段分析段落，每段含具體數據（月趨勢 X%、週環比 Y%）+ 因果推理 + KB 引用。Section 二和七保持中等長度。
...
**Section 字數均衡規範**：7 個 section（一～七）的字數應盡量均衡，每個 section 目標 400-600 字（去除空白）。避免 Section 一或 Section 六過長而其他 section 過短。若某 section 資料較少，增加推理深度和業界研究引用來補充；若某 section 過長，精簡為最重要的 3-4 個重點。
