...
**段落密度規範**：每段限 1-2 句（約 80-120 字），段落之間用空行分隔。每段至少包含一個數據點（百分比如 +5.9% 或絕對數字如 63,673,783）。不要將多個指標合併在同一段落中——每個指標或觀察拆為獨立段落。目標：全文 ≥60 個含數據的段落。

**雙時間框架規範**：提及任何指標時，必須在同一段落內同時寫出月趨勢和週環比兩個百分比。格式：「{指標名} 月趨勢 {X}%、週環比 {Y}%（latest {數值}）」。每個 section 至少 3 段含雙時間框架數據。目標：全文 ≥15 段含「月...%」+「週...%」的段落。
...
**Section 深度均衡規範**：Section 三（技術 SEO）、四（檢索未索引）、五（搜尋意圖）往往比其他 section 短，需增加推理深度：每個至少 3 段分析段落，每段含具體數據（月趨勢 X%、週環比 Y%）+ 因果推理 + KB 引用。Section 一和六不需壓縮。

**跨指標因果分析規範**：分析段落中必須大量使用因果連接詞（導致、因此、因為、進而、由於、造成、然而、儘管、所以、反映、顯示、源自、歸因、以致），將不同指標之間的因果關係串連起來。每個 section 至少 2 句含因果連接詞的跨指標推理。例：「曝光 +31.8% 但 CTR -10.9%，**導致**點擊成長幅度（+20.3%）遠低於曝光成長，**反映**新增觸及主要來自低排名位置。」


> **三層語意模型**（源自 claude-reports 資訊架構）：
> - **Tier 1 — Signal**（Section 一+二）：回答「本週 SEO 怎麼了？」30 秒掌握
> - **Tier 2 — Diagnosis**（Section 三+四+五）：回答「根因是什麼？」按需深入
> - **Tier 3 — Action**（Section 六+七）：回答「該做什麼？」行動導向
>
> Tier 1 應最精煉密實，Tier 2 可按需展開分析深度。
...
### Step C3：自我驗證（Self-Validation）

在存檔前，逐項檢查以下項目（不對使用者輸出，僅內部確認並修正）：

| 項目 | 標準 | 不達標行動 |
|------|------|-----------|
| Section 完整性 | 7 個 section 標題齊全 | 補齊缺少的 section |
| 雙時間框架密度 | ≥15 段含「月...%」+「週...%」| 補充缺少雙框架的段落 |
| 因果連接詞密度 | 每 section ≥2 句含因果詞 | 改寫為因果推理句 |
| Citation 標記 | [N] 標記 ≥8 筆 | 補引用 qaMap 相關 QA |
| ALERT 覆蓋率 | ≥50% ALERT_DOWN 在 S6 被提及 | 補行動項目 |
| 行動項目分佈 | 🔴≥4、🟡≥3、🟢≥2 | 調整優先級分類 |
| 段落密度 | ≥60 含數據段落 | 擴展分析深度 |
| Section 深度均衡 | S3/S4/S5 各 ≥3 段分析 | 補充推理段落 |

驗證方式：逐項計數，任何不達標項在存檔前修正。此步驟確保輸出品質穩定，不因指標組合變化而劣化。
...
```
<!-- report_meta {"weeks":1,"generated_at":"<UTC_ISO_TIME>","generation_mode":"claude-code","generation_label":"Claude Code 語意推理","model":null,"snapshot_id":"20260306-081902"} -->
```

- `weeks`：快照的 weeks 值（整數）
- `generated_at`：**必須是 UTC 時間**（用 bash 取得，見下方）
- `generation_mode`：固定 `"claude-code"`
- `generation_label`：固定 `"Claude Code 語意推理"`

接著計算 hash 和 UTC 時間（在 bash 中執行）：

```bash
# 取得 UTC 時間（必須用這個指令，不要手動填寫）
