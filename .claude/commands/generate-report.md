...
**若 API 不可用或輸入為 Google Sheets URL**（fallback）：跳過 Step C2，改為繼續執行下方完整 7 section 本地生成。
...
在生成本 section 前：
1. 讀取最新的 `output/meeting_prep_*.md`（若存在）
2. 解析 `<!-- meeting_prep_meta -->` 中的 `scores.maturity`
3. 行動建議依成熟度等級分層：
...
```
🔴 高優先（需立即處理）：針對 ALERT_DOWN，各一條具體行動 + KB 佐證連結 + 成熟度升級標籤
...
```
<!-- report_meta {"weeks":1,"generated_at":"2026-03-06T12:00:00.000Z","generation_mode":"claude-code","generation_label":"Claude Code 語意推理","model":null} -->
```
