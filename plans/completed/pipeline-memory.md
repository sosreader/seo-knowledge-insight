...
| #   | 任務                                                                                | 檔案                                | 狀態  | 實際          |
| --- | ----------------------------------------------------------------------------------- | ----------------------------------- | ----- | ------------- |
| 1   | 建立 `utils/pipeline_cache.py`                                                      | 新增                                | 1h    | ✅ 完成       |
| 2   | 建立 `utils/pipeline_version.py`                                                    | 新增                                | 1h    | ✅ 完成       |
| 3   | Step 2 整合 cache（`process_single_meeting`）                                       | `scripts/02_extract_qa.py`          | 1h    | ✅ 完成       |
| 4   | Step 3 整合 embedding cache（`get_embeddings`）                                     | `utils/openai_helper.py`            | 1h    | ✅ 完成       |
| 5   | Step 3 整合 classify cache（`classify_qa`）                                         | `utils/openai_helper.py`            | 30min | ✅ 完成       |
| 6   | Step 3 完成後 `record_artifact`                                                     | `scripts/03_dedupe_classify.py`     | 30min | ✅ 完成       |
| 7   | Step 4 整合 report cache + `record_artifact`                                        | `scripts/04_generate_report.py`     | 1h    | ✅ 完成       |
| 8   | `.gitignore` 加入 `output/.cache/`、`output/.versions/*.json`（保留 registry.json） | `.gitignore`                        | 5min  | ✅ 完成       |
| 9   | `make cache-stats` / `make version-history` Makefile target                         | `Makefile`                          | 30min | ✅ 完成       |
| 10  | 測試：相同輸入跑兩次，驗證 cache hit + token_used=0                                 | `tests/test_pipeline_cache.py`      | 1.5h  | ✅ 完成 49/49 |
| 11  | 新增 Step 2 `record_artifact(step=2)` 呼叫，含 early-exit 路徑                      | `scripts/02_extract_qa.py` L235-245 | 20min | ✅ 完成       |
...
> — Benet, J. (2014). _IPFS - Content Addressed, Versioned, P2P File System_. arXiv:1407.3561
...
> Zhuang, F. et al. (2023). _GPTCache: An Open-Source Semantic Cache for LLM Applications Enabling Faster Answers and Cost Savings_. https://github.com/zilliztech/GPTCache
...
> Anthropic. (2024). _Prompt Caching_. https://docs.anthropic.com/en/docs/build-with-claude/prompt-caching
...
> Yao, J. et al. (2024). _A Survey on Efficient Inference for Large Language Models_. arXiv:2404.14294
...
> Zaharia, M. et al. (2018). _Accelerating the Machine Learning Lifecycle with MLflow_. IEEE Data Eng. Bull. 41(4): 39–45.
...
> Ruslan Kuprieiev et al. (2020). _DVC: Data Version Control - Git for Data & Models_. https://dvc.org

**ModelDB（2016, MIT CSAIL）**

ModelDB 是最早提出「ML pipeline 版本管理」的學術系統，實驗顯示版本化不僅節省重複計算，也使除錯時間降低約 50%（由於可精確還原任意歷史版本的輸入輸出對）。

> Vartak, M. et al. (2016). _ModelDB: A System for Machine Learning Model Management_. HILDA @ SIGMOD 2016.
...
> Kleppmann, M. (2017). _Designing Data-Intensive Applications_. O'Reilly Media. Chapter 11.
...
> Beauchemin, M. (2018). _Functional Data Engineering — A Modern Paradigm for Batch Data Processing_. Medium / Lyft Engineering Blog. https://maximebeauchemin.medium.com/functional-data-engineering-a-modern-paradigm-for-batch-data-processing-2327ec32c42a
...
> Lewis, P. et al. (2020). _Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks_. NeurIPS 2020. arXiv:2005.11401
...
> _具體論文細節仍在 preprint 階段，可追蹤 arXiv cs.IR 分類。核心結論與 GPTCache benchmark 一致。_
...
## 十二、Phase 1 實現摘要（2026-02-28）

### ✅ 已完成的工作

**核心基礎設施**

- `utils/pipeline_cache.py` (130 行)
  - SHA256 content-addressed disk cache
  - 支援 5 個 namespace：extraction, embedding, classify, merge, report
  - 方法：cache_get, cache_set, cache_exists, cache_stats, cache_clear

- `utils/pipeline_version.py` (184 行)
  - Immutable artifact registry（output/.versions/step{N}/）
  - Content hash 冪等性設計
  - 方法：record_artifact, get_latest_version, get_version_history, get_all_token_stats

**Pipeline 整合（全數完成）**

- Step 2：extraction cache + record_artifact(step=2)
- Step 3：embedding/classify/merge cache + record_artifact(step=3)
- Step 4：report cache + record_artifact(step=4)

**版本管理**

- `.gitignore`：output/.cache/, output/.versions/step\*/
- Makefile targets：cache-stats, cache-clear, version-history
...
| -------- | -------- | -------- | -------------- |
| Step 2   | 150K     | **0**    | ~5K/新會       |
| Step 3   | 90K      | **0**    | ~300/新QA      |
| Step 4   | 8K       | **0**    | 0/週           |
| **合計** | **248K** | **0**    | **~5-6K/增量** |

### 🚀 Phase 2–3（待觸發）

- Phase 2：API 多 instance 或高頻查詢時（無急迫性）
- Phase 3：進階功能（Dashboard、Diff、LRU）

---

_計畫產出日期：2026-02-28_
_基於 seo-knowledge-insight 架構（單一 FastAPI + CLI pipeline）_
_前置文件：`plans/active/cache-redis.md`（Redis 模式詳細說明）_
