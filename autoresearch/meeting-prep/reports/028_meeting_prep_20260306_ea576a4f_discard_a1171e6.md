...
> 生成時間：2026-03-22
> 模式：claude-code（autoresearch round 028）
...
| # | 假設 | 層級 | 驗證方式 |
|---|------|------|---------|
| H5a | 檢索未索引月 +32.2%：/salon/ 大量新增 UGC 頁面未通過品質門檻 [8] | L2 | **可驗證**：在 GSC 篩選 /salon/ 路徑的未索引數量 |
| H5b | 有效 Coverage -5.8%：/en/ 和低品質頁面被 Google 主動去索引，/en/ hreflang 設定問題導致 canonical 衝突 | L1 | **可驗證**：在 Screaming Frog 驗證 /en/ hreflang 標籤 |
| H5c | /article/ 和 /tag/ 新頁面連結數不足，成為孤島頁面，索引優先級降低 [4] | L2 | **可驗證**：在 Screaming Frog 分析 /article/ 和 /tag/ 頁面連結數 |

### H6：KW: 影評 / KW: 電影 / KW: 股

| # | 假設 | 層級 | 驗證方式 |
|---|------|------|---------|
| H6a | KW: 影評 +20%、KW: 電影 +56.1% 是娛樂類自然成長，Video +340% 強化影片 SERP Feature 帶動 | L3 | **可驗證**：在 GSC 篩選影評和電影查詢的 CTR 和排名位置 |
| H6b | KW: 股 +72% 受財經行情帶動，Google News 訂閱 +90.6% 可能強化財經類曝光 | L5 | **可驗證**：在 Google Trends 查看「股」近 30 天搜尋量 |
| H6c | Video Appearance +340% 和影片索引 +5836.4% 代表 VideoObject 結構化資料策略生效 [14] | L1 | **可驗證**：在 GSC 篩選影片 URL 索引狀態 |
...
- [ ] [B1] 檢索未索引 +32.2% 且內部連結分布 -16.3%——在 Screaming Frog 分析 /salon/ 頁面平均連結數是否 < 3？（來源：S7 連結生態，評分 2/5）
...
- [ ] [B4] KW: 影評 +20% 和 KW: 電影 +56.1%——在 GSC 確認娛樂類 KW 的 CTR 和 Discover 推薦趨勢？（來源：S7 趨勢敏銳度，評分 3/5）
- [ ] [B5] KW: 股 +72% 且 Discover 週 +17.7%——財經類內容如何複製受益模式？（來源：S7 內容靈魂，評分 3/5）
